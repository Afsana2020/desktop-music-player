...
function App() {
  ...
  useEffect(() => {
    async function loadDefaultSongs() {
      if (window.electronAPI && window.electronAPI.getDefaultSongs) {
        try {
          const defaults = await window.electronAPI.getDefaultSongs();
          const formatted = defaults.map((song, index) => ({
            ...song,
          ...
          setSongs(formatted);
          if (formatted.length > 0) setCurrentSongIndex(0);
        } catch (err) {
